...
import { Accordion, AccordionDetails, AccordionSummary, Avatar, Backdrop, Badge, Box, Card, CardContent, CardHeader, Chip, CircularProgress, Divider, FormControl, FormLabel, Grid, Hidden, IconButton, InputAdornment, InputLabel, ListItem, MenuItem, Paper, Popover, Select, SelectChangeEvent, Stack, Table, TableBody, TableCell, TableContainer, TableRow, TextField, Tooltip, Typography } from "@mui/material";
import { AccountBalanceWallet, AccountCircle, Add, AddShoppingCartOutlined, ArrowDropDown, BadgeOutlined, CameraRoll, Delete, InfoOutlined, Key, Label, MoreVert, UnfoldMoreOutlined } from "@mui/icons-material";
import { useSnackbar } from "notistack";
import { TransactionInvalidBeaconError } from "./TransactionInvalidBeaconError";
import {   ContractFA12Storage,  ContractStorage } from "./TicketerContractUtils";
import {  getBytes, LAYER2Type, RollupDEKU, RollupTORU, ROLLUP_TYPE, TezosUtils, TOKEN_TYPE } from "./TezosUtils";
...
const handleWithdraw = async (event : MouseEvent<HTMLButtonElement>) => {
    ...
    try {
        ...
        rollupType === ROLLUP_TYPE.DEKU ? new RollupParametersDEKU(process.env["REACT_APP_CONTRACT"]!+"%withdrawDEKU", quantity,tokenType == TOKEN_TYPE.XTZ ? await getBytes(TOKEN_TYPE.XTZ) : await getBytes(TOKEN_TYPE.CTEZ,process.env["REACT_APP_CTEZ_CONTRACT"]!) ,handleId,l1Address,process.env["REACT_APP_CONTRACT"]!,proof,proofList) 
...
return (
    ...
    InputProps={{
        ...
        value={quantity}
        label={<span>Quantity <Tooltip title="ticket quantity is expressed with the lowest decimal. Ex : mutez, muctez, etc ..."><InfoOutlined/></Tooltip></span> }
        inputProps={{style: { textAlign: 'right' }}} 
        InputProps={{
            startAdornment: (
                <InputAdornment position="start">
