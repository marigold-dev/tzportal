...
import {
  ContractFAParameters,
  ContractFAStorage,
  ContractParameters,
  ContractStorage,
  ContractXTZParameters,
} from "./TicketerContractUtils";
...
type DepositWithdrawV2Props = {
  ...
  setRollup: Dispatch<
    SetStateAction<RollupTORU | RollupDEKU | RollupCHUSAI | undefined>
  >;
...
const DepositWithdrawV2 = ({
  ...
  useEffect(() => {
    ...
    oldTicketBalance.current = userTicketBalance.get(
      TOKEN_TYPE[tokenType as keyof typeof TOKEN_TYPE]
    )!;
  ...
  const isDirectionDeposit = () => {
    return (
      ...
      activeAccount.accountIdentifier !== LAYER2Type.L2_DEKU
    );
  };
  ...
  const switchActiveAccount = async () => {
    ...
    const l2Account: AccountInfo | undefined = accounts.find((a) => {
      return (
        a.address == userL2Address && a.accountIdentifier === LAYER2Type.L2_DEKU
      );
    });
    if (
      activeAccount?.address === l1Account?.address &&
      activeAccount?.accountIdentifier !== LAYER2Type.L2_DEKU
    ) {
  ...
  const refreshBalance = async () => {
    ...
    //kUSD
    let kUSDContract = await Tezos.wallet.at(
    ...
    //CTEZ
    let ctezContract = await Tezos.wallet.at(
    ...
    //UUSD
    let uusdContract = await Tezos.wallet.at(
    ...
    //EURL
    let eurlContract = await Tezos.wallet.at(
  ...
  const refreshTicketBalance = async () => {
    ...
    const XTZbalance = new BigNumber(
      await dekuClient.getBalance(userL2Address, {
        ticketer: process.env["REACT_APP_CONTRACT"]!,
        data: tokenBytes.get(TOKEN_TYPE.XTZ)!,
      })
    );
    ...
    let kUSDBalance = new BigNumber(
      await dekuClient.getBalance(userL2Address, {
        ticketer: process.env["REACT_APP_CONTRACT"]!,
        data: tokenBytes.get(TOKEN_TYPE.KUSD)!,
      })
    );
    ...
    let ctezBalance = new BigNumber(
      await dekuClient.getBalance(userL2Address, {
        ticketer: process.env["REACT_APP_CONTRACT"]!,
        data: tokenBytes.get(TOKEN_TYPE.CTEZ)!,
      })
    );
    ...
    let uusdBalance = new BigNumber(
      await dekuClient.getBalance(userL2Address, {
        ticketer: process.env["REACT_APP_CONTRACT"]!,
        data: tokenBytes.get(TOKEN_TYPE.UUSD)!,
      })
    );
    ...
    let eurlBalance = new BigNumber(
      await dekuClient.getBalance(userL2Address, {
        ticketer: process.env["REACT_APP_CONTRACT"]!,
        data: tokenBytes.get(TOKEN_TYPE.EURL)!,
      })
    );
    ...
    balance.set(
      TOKEN_TYPE.KUSD,
      kUSDBalance.dividedBy(
        Math.pow(10, (await kUSDContract.tzip12().getTokenMetadata(0)).decimals)
      )
    ); //convert from lowest kUSD decimal
    balance.set(
      TOKEN_TYPE.CTEZ,
      ctezBalance.dividedBy(
        Math.pow(10, (await ctezContract.tzip12().getTokenMetadata(0)).decimals)
      )
    ); //convert from muctez
    balance.set(
      TOKEN_TYPE.UUSD,
      uusdBalance.dividedBy(
        Math.pow(10, (await uusdContract.tzip12().getTokenMetadata(0)).decimals)
      )
    ); //convert from lowest UUSD decimal
    balance.set(
      TOKEN_TYPE.EURL,
      eurlBalance.dividedBy(
        Math.pow(10, (await eurlContract.tzip12().getTokenMetadata(0)).decimals)
      )
    ); //convert from lowest EURL decimal
    ...
    if (!oldTicketBalance.current) {
      ...
      setTimeout(() => {
        rollupBoxRef?.current?.setChangeTicketColor(
          newCurrentBalance.isGreaterThan(oldTicketBalance.current!)
            ? "green"
            : "red"
        );
  ...
  const isDepositButtonDisabled = (): boolean | undefined => {
    ...
    switch (tokenType) {
      case TOKEN_TYPE.XTZ:
        isDisabled =
          quantity.isZero() ||
          (userBalance.get(TOKEN_TYPE.XTZ) !== undefined &&
            userBalance.get(TOKEN_TYPE.XTZ)!.isLessThan(quantity));
        break;
      case TOKEN_TYPE.CTEZ:
        isDisabled =
          quantity.isZero() ||
          (userBalance.get(TOKEN_TYPE.CTEZ) !== undefined &&
            userBalance.get(TOKEN_TYPE.CTEZ)!.isLessThan(quantity));
        break;
      case TOKEN_TYPE.KUSD:
        isDisabled =
          quantity.isZero() ||
          (userBalance.get(TOKEN_TYPE.KUSD) !== undefined &&
            userBalance.get(TOKEN_TYPE.KUSD)!.isLessThan(quantity));
        break;
      case TOKEN_TYPE.UUSD:
        isDisabled =
          quantity.isZero() ||
          (userBalance.get(TOKEN_TYPE.UUSD) !== undefined &&
            userBalance.get(TOKEN_TYPE.UUSD)!.isLessThan(quantity));
        break;
      case TOKEN_TYPE.EURL:
        isDisabled =
          quantity.isZero() ||
          (userBalance.get(TOKEN_TYPE.EURL) !== undefined &&
            userBalance.get(TOKEN_TYPE.EURL)!.isLessThan(quantity));
        break;
    }
    return isDisabled;
  };

  const handlePendingDeposit = async (
    event: MouseEvent<HTMLButtonElement>,
    from: string,
    contractFAStorage: ContractFAStorage,
    ticketTokenType: string
  ) => {
    ...
    try {
      ...
      //1.a for FA1.2
      if (
        ticketTokenType === TOKEN_TYPE.CTEZ ||
        ticketTokenType === TOKEN_TYPE.KUSD
      ) {
        let fa12Contract: WalletContract = await Tezos.wallet.at(
          contractFAStorage.faAddress
        );
        operations.push({
          kind: OpKind.TRANSACTION,
          ...fa12Contract.methods
            .transfer(
              from,
              contractStorage?.treasuryAddress,
              contractFAStorage.amountToTransfer.toNumber()
            )
            .toTransferParams(),
        });
      }
      //1.B for FA2
      if (
        ticketTokenType === TOKEN_TYPE.UUSD ||
        ticketTokenType === TOKEN_TYPE.EURL
      ) {
        let fa2Contract: WalletContract = await Tezos.wallet.at(
          contractFAStorage.faAddress
        );
        operations.push({
          kind: OpKind.TRANSACTION,
      ...
      enqueueSnackbar(
        "Treasury has batched collaterization " +
          contractFAStorage.amountToTransfer.toNumber() +
          " tokens from " +
          from,
    ...
    try {
      ...
      let l2Type: LAYER2Type =
        contractFAStorage.l2Type.l2_TORU &&
        contractFAStorage.l2Type.l2_TORU !== ""
          ? LAYER2Type.L2_TORU
          : contractFAStorage.l2Type.l2_DEKU &&
            contractFAStorage.l2Type.l2_DEKU !== ""
          ? LAYER2Type.L2_DEKU
          : LAYER2Type.L2_CHUSAI;
      const param =
        l2Type == LAYER2Type.L2_TORU
          ? {
              "address": from,
      ...
      operations.push({
        kind: OpKind.TRANSACTION,
        ...contract!.methods
          .pendingDeposit(...Object.values(param))
          .toTransferParams(),
      });
      ...
      enqueueSnackbar(
        "Pending deposit from " + from + " has been successfully processed",
        { variant: "success", autoHideDuration: 10000 }
      );
    } catch (error: any) {
      console.table(`Error: ${JSON.stringify(error, null, 2)}`);
  ...
  const handlePendingWithdraw = async (
    ...
    try {
      ...
      operations.push({
        kind: OpKind.TRANSACTION,
        ...contract!.methods
          .withdrawPendingDEKU(...Object.values(param))
          .toTransferParams(),
      });

      enqueueSnackbar(
        "Pending withdraw for " + to + " has been successfully batched",
        { variant: "success", autoHideDuration: 10000 }
      );
    } catch (error: any) {
      console.table(`Error: ${JSON.stringify(error, null, 2)}`);
  ...
  const calcMinBuyValue = (
    amount: number,
    cfmmStorage: CfmmStorage
  ): number => {
    if (cfmmStorage) {
      ...
      const tokWithoutSlippage =
        (cashSold * 997 * aPool.toNumber()) /
        (bPool.toNumber() * 1000 + cashSold * 997) /
        1e6;
  ...
  const handleDeposit = async (event: MouseEvent) => {
    ...
    let c: WalletContract = await Tezos.wallet.at(
    ...
    try {
      if (tokenType === TOKEN_TYPE.XTZ && rollupType === ROLLUP_TYPE.DEKU) {
        enqueueSnackbar(
          "We need to swap XTZ against CTEZ first, then deposit your CTEZ",
          { variant: "warning", autoHideDuration: 5000 }
        );

        let faContract = await Tezos.wallet.at(
          process.env["REACT_APP_CTEZ_CONTRACT"]!,
          compose(tzip12, tzip16)
        );
        let faSwapContract = await Tezos.wallet.at(
          process.env["REACT_APP_CTEZ_SWAP_CONTRACT"]!
        );
        const storage = await faSwapContract.storage<CfmmStorage>();

        const decimals = Math.pow(
          10,
          (await faContract.tzip12().getTokenMetadata(0)).decimals
        );
        const now = new Date();

        //FIXME , we need the current XTZ/CTEZ price to reserve enough XTZ to pay
        const op = await faSwapContract.methods
          .cashToToken(
            userAddress,
            calcMinBuyValue(quantity.toNumber(), storage) * decimals,
            new Date(now.setMinutes(now.getMinutes() + 1)).toISOString()
          )
          .send({ amount: quantity.toNumber() });

        await op.confirmation(1);

        enqueueSnackbar("You can use your new CTEZ to do your deposit now", {
          variant: "success",
          autoHideDuration: 10000,
        });
      ...
      if (tokenType === TOKEN_TYPE.CTEZ || tokenType === TOKEN_TYPE.KUSD) {
        let faContract = await Tezos.wallet.at(
          tokenType === TOKEN_TYPE.CTEZ
            ? process.env["REACT_APP_CTEZ_CONTRACT"]!
            : process.env["REACT_APP_KUSD_CONTRACT"]!,
          compose(tzip12, tzip16)
        );
        let faContractStorage: FA12Contract =
          (await faContract.storage()) as FA12Contract;
        let allowance: BigNumber | undefined =
          await faContractStorage.allowances.get<BigNumber>({
            owner: userAddress,
        ...
        if (
          allowance === undefined ||
          allowance.isLessThan(quantity.multipliedBy(decimals))
        ) {
          enqueueSnackbar(
            "Allowance (" +
              allowance +
              ") is not enough for requested collateral of " +
              quantity.multipliedBy(decimals) +
              ", please allow an allowance first",
          ...
          if (allowance === undefined || allowance.toNumber() == 0) {
            ...
            enqueueSnackbar(
              "Your allowance of " +
                ...
                " " +
                tokenType +
                " has been batched for Treasury (wait 1 or 2 blocks for Treasury to take your collateral)",
              { variant: "success", autoHideDuration: 10000 }
            );
          } else {
            //need to reset allowance to zero, then reset allowance back to avoid HACK
            enqueueSnackbar(
              "As allowance is not null, we need to reset allowance to zero, then reset allowance back to quantity to avoid HACK",
              { variant: "warning", autoHideDuration: 10000 }
            );
            ...
            operations.push({
              kind: OpKind.TRANSACTION,
              ...faContract.methods
                .approve(contractStorage?.treasuryAddress, 0)
                .toTransferParams(),
            });
          ...
          console.log(
            "FA1.2 allowance is fine (actual : " +
              allowance.dividedBy(decimals).toNumber() +
              ", requested : " +
              quantity +
              ")"
          );
      ...
      if (tokenType === TOKEN_TYPE.EURL || tokenType === TOKEN_TYPE.UUSD) {
        let fa2Contract = await Tezos.wallet.at(
          tokenType === TOKEN_TYPE.UUSD
            ? process.env["REACT_APP_UUSD_CONTRACT"]!
            : process.env["REACT_APP_EURL_CONTRACT"]!,
          tzip12
        );
        ...
        decimals = Math.pow(
          10,
          (await fa2Contract.tzip12().getTokenMetadata(0)).decimals
        );

        console.log(
          "parameter signature",
          fa2Contract.parameterSchema.ExtractSignatures()
        );
        ...
        enqueueSnackbar(
          "Treasury " +
            contractStorage?.treasuryAddress +
            " has been added to operator list",
          { variant: "success", autoHideDuration: 10000 }
        );
      }
      ...
      let param: ContractParameters =
        tokenType === TOKEN_TYPE.XTZ
          ? new ContractXTZParameters(
              quantity.multipliedBy(decimals),
              rollupType === ROLLUP_TYPE.DEKU
                ? LAYER2Type.L2_DEKU
                : rollupType === ROLLUP_TYPE.TORU
                ? LAYER2Type.L2_TORU
                : LAYER2Type.L2_CHUSAI,
              userL2Address,
      ...
      enqueueSnackbar(
        tokenType === TOKEN_TYPE.XTZ
          ? "Your deposit has been accepted (wait 1 or 2 blocks until your L2 balance get refreshed)"
          : "Your deposit is in pending, waiting for Treasury to get your tokens in collateral and continue process",
        { variant: "success", autoHideDuration: 10000 }
      );

      if (rollupType === ROLLUP_TYPE.TORU) {
        //need to fecth the ticket hash to give it to the user
                  ...
                  if ("metadata" in content) {
                    ...
                    if (iorArray && iorArray.length > 0) {
                      for (const ior of iorArray) {
                        ticketHash = (ior.result as OperationResultTransaction)
                          .ticket_hash!;
                        break;
                      }
                    } else continue;
        ...
        enqueueSnackbar(
          "Store your ticket hash somewhere, you will need it later : " +
            ticketHash,
          { variant: "success", autoHideDuration: 10000 }
        );
      }
  ...
  const handleL2Withdraw = async (event: MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      if (tokenType !== TOKEN_TYPE.XTZ) {
        let faContract = await TezosL2.wallet.at(
          tokenType === TOKEN_TYPE.CTEZ
            ? process.env["REACT_APP_CTEZ_CONTRACT"]!
            : tokenType === TOKEN_TYPE.KUSD
            ? process.env["REACT_APP_KUSD_CONTRACT"]!
            : tokenType === TOKEN_TYPE.UUSD
            ? process.env["REACT_APP_UUSD_CONTRACT"]!
            : process.env["REACT_APP_EURL_CONTRACT"]!,
      ...
      enqueueSnackbar(
        "The proof will be available in 10s. Keep this code ( " +
          opHash +
          " ) to do a Claim on L1 with user " +
          userL2Address,
        { variant: "success", autoHideDuration: 10000 }
      );
    } catch (error: any) {
    ...
    >
      ...
      <Stack width="inherit">
        ...
        <Grid item xs={12} md={3}>
          {isDirectionDeposit() ? (
            ...
            <div style={{ height: 0 }}>
              <Button
                disabled={
                  rollupType === ROLLUP_TYPE.CHUSAI ||
                  rollupType === ROLLUP_TYPE.TORU
                }
                sx={{ position: "relative", top: "-90px" }}
                color="warning"
                variant="contained"
                onClick={(e) => handleL2Withdraw(e)}
              >
                Withdraw
              </Button>
            </div>
          ...
          {contractStorage?.treasuryAddress == userAddress ? (
            <Accordion>
              ...
              <AccordionDetails>
                {!isDirectionDeposit() && contractStorage.faPendingWithdrawals
                  ? Array.from(
                      contractStorage.faPendingWithdrawals.entries()
                    ).map(
                      ([key, val]: [[string, string], ContractFAStorage]) => {
                        let tokenType: string =
                          tokenBytes.get(TOKEN_TYPE.XTZ) == key[1]
                            ? TOKEN_TYPE.XTZ
                            : tokenBytes.get(TOKEN_TYPE.CTEZ) == key[1]
                            ? TOKEN_TYPE.CTEZ
                            : tokenBytes.get(TOKEN_TYPE.KUSD) == key[1]
                            ? TOKEN_TYPE.KUSD
                            : tokenBytes.get(TOKEN_TYPE.UUSD) == key[1]
                            ? TOKEN_TYPE.UUSD
                            : TOKEN_TYPE.EURL;

                        return (
                          <div key={key[0] + key[1] + val.type}>
                            <Badge
                              ...
                              for{" "}
                              {
                                <span className="address">
                                  <span className="address1">
                                    {key[0].substring(0, key[0].length / 2)}
                                  </span>
                                  <span className="address2">
                                    {key[0].substring(key[0].length / 2)}
                                  </span>
                                </span>
                              }{" "}
                            </span>
                ...
                {isDirectionDeposit() && contractStorage.faPendingDeposits
                  ? Array.from(contractStorage.faPendingDeposits.entries()).map(
                      ([key, val]: [[string, string], ContractFAStorage]) => {
                        let l2Address: string = val.l2Type.l2_DEKU
                          ? val.l2Type.l2_DEKU
                          : val.l2Type.l2_TORU;
                        let tokenType: string =
                          tokenBytes.get(TOKEN_TYPE.XTZ) == key[1]
                            ...
                            <span>
                              ...
                              for{" "}
                              {
                                <span className="address">
                                  ...
                                  <span className="address2">
                                    {l2Address.substring(l2Address.length / 2)}
                                  </span>
                                </span>
                              }{" "}
                            </span>
        ...
        <Grid item xs={12} md={1}>
          <Divider orientation="horizontal" sx={{ display: "block" }}>
            {" "}
            <SwapVert
              sx={{ transform: "scale(3.5)" }}
              color="primary"
              onClick={() => switchActiveAccount()}
            />
          </Divider>
        </Grid>
