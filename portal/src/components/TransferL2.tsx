...
const TransferL2 = ({
  ...
  const [userL2DestinationAddress, setUserL2DestinationAddress] =
    useState<string>(""); // L2 user destination address
  ...
  >(new Map());
  const [userTicketDestinationBalance, setUserTicketDestinationBalance] =
    useState<Map<TOKEN_TYPE, BigNumber>>(new Map()); //the L2 destination address
  ...
  useEffect(() => {
    ...
    oldTicketDestinationBalance.current = userTicketDestinationBalance.get(
      TOKEN_TYPE[tokenType as keyof typeof TOKEN_TYPE]
    )!;
  ...
  const refreshTicketBalance = async (destinationL2Address?: string) => {
    ...
    let userL2: string = destinationL2Address
      ? destinationL2Address
      : userL2Address;
    ...
    //kUSD
    let kUSDContract = await TezosL2.wallet.at(
    ...
    //CTEZ
    console.log(
      "Refreshing balance for Ctez: ",
      process.env["REACT_APP_CTEZ_CONTRACT"],
      tokenBytes.get(TOKEN_TYPE.CTEZ)
    );
    let ctezContract = await TezosL2.wallet.at(
      process.env["REACT_APP_CTEZ_CONTRACT"]!,
      compose(tzip12, tzip16)
    );
    ...
    //UUSD
    let uusdContract = await TezosL2.wallet.at(
    ...
    //EURL
    let eurlContract = await TezosL2.wallet.at(
    ...
    destinationL2Address
      ? setUserTicketDestinationBalance(balance)
      : setUserTicketBalance(balance);
    console.log(
      "[TransferL2] All ticket balances initialized for " + userL2,
      balance
    );

    destinationL2Address
      ? rollupDestinationBoxRef?.current?.setShouldBounce(false)
      : rollupBoxRef?.current?.setShouldBounce(false);

    if (destinationL2Address) {
      if (!oldTicketDestinationBalance.current) {
        //first time, we just record the value
        oldTicketDestinationBalance.current = newCurrentBalance;
      } else if (
        !newCurrentBalance.isEqualTo(oldTicketDestinationBalance.current)
      ) {
        setTimeout(() => {
          rollupDestinationBoxRef?.current?.setChangeTicketColor(
            newCurrentBalance.isGreaterThan(
              oldTicketDestinationBalance.current!
            )
              ? "green"
              : "red"
          );
  ...
  useEffect(() => {
    ...
    const intervalId2 = setInterval(
      () => refreshTicketBalance(userL2DestinationAddressRef.current),
      15 * 1000
    ); //refresh async L2 destination balances
    ...
    <Box
      ...
      sx={
        isDesktop
          ...
          : { marginTop: 0, padding: 0, borderRadius: "0" }
      }
    >
      <Backdrop
      ...
      <Stack
        sx={isDesktop ? { width: "inherit" } : { padding: "30px" }}
        direction="column"
        spacing={2}
      >
        <RollupBox
