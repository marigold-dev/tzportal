...
let cash_to_token (param : cash_to_token) (storage : storage) =
   let { to_ = to_ ;
    ...
    if storage.pendingPoolUpdates > 0n then
        ...
        let _cashPool = storage.cashPool in
        (* 1:1 peg cheat *)
        let tokens_bought =
            (let bought = cashSold in    
...
(* Entrypoint to allow depositing tez. *)
    ...
    if storage.pendingPoolUpdates > 0n then
        ...
        let storage = {storage with cashPool = storage.cashPool + mutez_to_natural (Tezos.get_amount ()) } in
...
let accept_manager (storage : storage) : result =
    if storage.pendingPoolUpdates > 0n then
        (failwith error_PENDING_POOL_UPDATES_MUST_BE_ZERO : result)
    else if Tezos.get_sender () <> storage.new_manager then
...
let tez_to_token (_param : tez_to_token) (storage : storage) : result =
        ([] , storage)
#endif
